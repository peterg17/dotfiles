...
- Claude-only:
  ...
  - **`obsidian-ticket-create`** — write a new Obsidian ticket note in the same schema as pi's `obsidian_ticket_create` tool (frontmatter + Problem/Acceptance/Context/Agent Instructions/Work Log/PR sections). Notes round-trip with pi's `obsidian-tickets` extension.
