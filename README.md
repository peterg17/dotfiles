...
- Claude-only:
  ...
  - **`obsidian-ticket-team`** — Claude counterpart to the pi skill of the same name. Spawns a team for implementation work described by an Obsidian Markdown ticket note (or 2-5 related notes); team lead updates the note's frontmatter and `## Work Log` as the implementer, reviewer, and tester progress.
